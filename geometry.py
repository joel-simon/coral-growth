...
def point_in_polygon(pt, poly, inf=750):
    result = False
    for i in range(len(poly)-1):
        if intersect(Vector(poly[i].x, poly[i].y), Vector( poly[i+1].x, poly[i+1].y), Vector(pt.x, pt.y), Vector(inf, pt.y)):
            result = not result
    if intersect(Vector(poly[-1].x, poly[-1].y), Vector(poly[0].x, poly[0].y), Vector(pt.x, pt.y), Vector(inf, pt.y)):
        result = not result
    return result

def lineIntersection (p1, p2, p3, p4):
    s = ((p4.x - p3.x) * (p1.y - p3.y) - (p4.y - p3.y) * (p1.x - p3.x)) / ((p4.y - p3.y) * (p2.x - p1.x) - (p4.x - p3.x) * (p2.y - p1.y))
    return Vector(p1.x + s * (p2.x - p1.x), p1.y + s * (p2.y - p1.y))
